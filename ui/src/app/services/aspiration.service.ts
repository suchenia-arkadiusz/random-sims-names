...
export class AspirationService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.http.get<Aspiration[]>(`${this.MAIN_URL}/aspirations`)
  ...
  addNewAspiration(aspirations: Aspiration[]) {
    this.http.post<Aspiration[]>(`${this.MAIN_URL}/aspirations`, aspirations)
      .subscribe((response) => {
  ...
  deleteAspiration(aspiration: Aspiration) {
    ...
    if (index > -1) {
      this.http.delete<Aspiration[]>(`${this.MAIN_URL}/aspirations/${aspiration.name}`)
        .subscribe((response) => {
          this.aspirations.next(response);
        }, error => console.error(error));
    }
